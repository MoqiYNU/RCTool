 ...
 *         createArcPoint,
 *         createTransitionElement methods it uses to a separate DataLayerWriter
 *         class,
 *         as part of refactoring to remove XML related actions from the
 *         DataLayer class.
 * 
 ...
 *         class. The
 *         calls to actually populate a DataLayer object with the info contained
 *         in the
 *         PNML document have been moved to a createFromPNML method. The
 *         DataLayer
 *         constructor which previously used loadPNML has been changed to
 *         reflect
 *         these modifications. Also moved getDOM methods to PNMLTranformer
 *         class, as
 *         getDom is XML related. Removed getDom() (no arguments) completely as
 *         this
 ...
 *         (just fire
 *         it, don't get a random variable first). Also, throw a
 *         RuntimeException if a
 *         rate less than 1 is detected. The current code uses the rate as a
 *         weight,
 *         and a rate such as 0.5 leads to a condition like that of bug 1699546
 *         where no
 ...
 *         RuntimeException
 *         thrown now. The code for timed transitions uses the same logic, but
 *         will soon
 *         be changed to use exponentially distributed times where fractional
 *         rates
 *         are valid.
 *
...
public class DataLayer
   ...
   // 多个消息库所
   private List<String> msgPlaces = null;

   // 多个资源库所
   private List<String> resPlaces = null;

   // 多个链接库所
   private List<String> linkPlaces = null;

   // 多个数据库所
   private List<String> dataPlaces = null;

   // 多个控制变迁
   private List<String> routTrans = null;
   ...
   private void initializeMatrices() {
      ...
      dataPlaces = new ArrayList<String>();
      routTrans = new ArrayList<String>();
   }
   ...
   public void addMsgPlace(String place) {
      if (!msgPlaces.contains(place)) {
         msgPlaces.add(place);
         // System.out.println("add: " + msgPlaces);
      }
   }

   public void removeMsgPlace(String place) {
      msgPlaces.remove(place);
      // System.out.println("rem: " + msgPlaces);
   }

   public boolean isMsgPlace(String placeId) {
      if (msgPlaces.contains(placeId)) {
         return true;
      } else {
         return false;
      }
   ...
   public void addResPlace(String place) {
   ...
   public void removeResPlace(String place) {
      resPlaces.remove(place);
      // System.out.println("rem: " + msgPlaces);
   }

   public boolean isResPlace(String placeId) {
      if (resPlaces.contains(placeId)) {
         return true;
      } else {
         return false;
      }
   ...
   public List<String> getRoutTrans() {
      return routTrans;
   }

   public void setRoutTrans(List<String> routTrans) {
      this.routTrans = routTrans;
   }

   public void addRoutTran(String routTran) {
      if (!routTrans.contains(routTran)) {
         routTrans.add(routTran);
      }
   }

   public void removeRoutTran(String routTran) {
      routTrans.remove(routTran);
   }

   public boolean isRoutTran(String tranId) {
      if (routTrans.contains(tranId)) {
         return true;
      } else {
         return false;
      }
   ...
   //创建初始标识
   public Marking createInitMarking() {
       ...
       for( ; placeNo < placeSize ; placeNo++){
     	  int curTokens = ((Place)placesArray.get(placeNo)).getCurrentMarking();
     	  if (curTokens != 0) {
     		  for (int i = 0; i < curTokens; i++) {
     			  //存储的是库所Id
     			  String placeId = ((Place)placesArray.get(placeNo)).getId();
     			  places.add(placeId);
     		  }
 		}
       }
       initMarking.setPlaces(places);
       return initMarking;
   }
   ...
   private AnnotationNote createAnnotation(Element inputLabelElement) {
      ...
      String finalMarkings = inputLabelElement.getAttribute("finalMarings");
      ...
      AnnotationNote annotationNote = new AnnotationNote(text, positionXInput, positionYInput,
            widthInput, heightInput, borderInput);

      if (finalMarkings.equals("true")) {// 若为终止标识标注,则设为true
         annotationNote.setFinalMarkingNote(true);
      }
   ...
   private Transition createTransition(Element element) {
      ...
      // 获取变迁是否是路由属性
      String routTran = element.getAttribute("routTran");
      if (routTran != "") {
    	  if (routTran.equals("true")) {// 添加路由变迁属性
    	         transition.setRoutTran(true);
    	         addRoutTran(transition.getId());
    	      }else {
    	    	  transition.setRoutTran(false);
			}
      }else {
    	  transition.setRoutTran(false);
      }
      ...
      String minDelay = element.getAttribute("minDelay");
      String maxDelay = element.getAttribute("maxDelay");
      
      // 如果在XML文件中不存在该元素      
      if (minDelay != "" && maxDelay != "") {
    	  //设置变迁最小和最大执行时间
          transition.setMinDelay(Double.parseDouble(minDelay));
          transition.setMaxDelay(Double.parseDouble(maxDelay));
      }else {
    	  //设置变迁最小和最大执行时间
          transition.setMinDelay(0);
          transition.setMaxDelay(0); 
      }
   ...
   private Place createPlace(Element element) {
      ...
      // 获取inputPlace的消息,资源和链接库所属性
      String msgPlace = element.getAttribute("msgPlace");
      String resPlace = element.getAttribute("resPlace");
      String linkPlace = element.getAttribute("linkPlace");
      String dataPlace = element.getAttribute("dataPlace");
      ...
      /*
       * //设置place的源库所,终止库所及消息库所属性
       * if (source.equals("true")) {
       * place.setSource(true);
       * addSource(place.getId());
       * }
       * if (sink.equals("true")) {
       * place.setSink(true);
       * addSink(place.getId());
      ...
      if (msgPlace.equals("true")) {
         place.setMsgPlace(true);
         addMsgPlace(place.getId());
      }
      if (resPlace.equals("true")) {
         place.setResPlace(true);
         addResPlace(place.getId());
      }
   ...
   private void createPriorityVector() {
      ...
      for (int transitionNo = 0; transitionNo < transitionSize; transitionNo++) {
         priorityVector[transitionNo] = ((Transition) transitionsArray.get(transitionNo)).getPriority();
      }
   }
   ...
   public void storeState() {
      ...
      for (int placeNo = 0; placeNo < placeSize; placeNo++) {
         markingVectorAnimationStorage[placeNo] = ((Place) placesArray.get(placeNo)).getCurrentMarking();
      }
   }
    ...
    * 
    * @return A List of all the net-level (as opposed to element-specific) label
    *         objects
    */
