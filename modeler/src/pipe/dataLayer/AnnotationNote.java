...
public class AnnotationNote extends Note {
   ...
   public AnnotationNote paste(double x, double y, boolean toAnotherView) {
      AnnotationNote anNote = new AnnotationNote (this.note.getText(),   
      ...
      //设置终止标识标注
      if (this.isFinalMarkingNote()) {
	      anNote.setFinalMarkingNote(true);
	  }
      return anNote;
   }   
   ...
   public void paintComponent(Graphics g) {
	...
	AffineTransform saveAT = g2.getTransform();
      ...
      if (isFinalMarkingNote()) {//若为终止标识标注,则设置边框为红色
