 ...
 *         are
 *         inverses of each other, so no matter how the parameter is specified,
 *         the
 *         code after the constructor is the same. When a timed transition is
 *         found to
 *         be enabled, a projected delay is calculated based on its exponential
 *         distribution. As long as the transition does not get disabled before
 *         it
 *         can be fired, that delay governs the timing of the transition's
 *         firing. If
 *         other timed transitions are fired, the delay before they fire (that
 *         is, the
 *         progress of the virtual clock) is decremented from this transition's
 *         delay
 *         time to simulate the progress of time in the virtual space. Thus, at
 *         any
 *         time, the next timed transition to fire is the one with the lowest
 *         delay
 *         remaining. If a timed transition gets an expected delay and then
 *         later becomes
 *         disabled, its delay is no longer valid. When it next becomes enabled,
 *         it will
 *         get a new expected delay (an offset from the current virtual time).
 *         This
 *         explaination was included to clarify any confusion about fields and
 *         methods
 *         with "delay" in their names. They have nothing to do with whether the
 *         parameter of the exponential distribution is specified as a rate, or
 *         an
 *         expected delay.
 *         to the
 */
...
public class Transition
   ...
   // 是否为路由变迁
   private boolean isRoutTran = false;
...
//   protected boolean attributesVisible = true;
   ...
   public Transition paste(double x, double y, boolean fromAnotherView) {
      ...
      if (this.isRoutTran) {// 添加控制变迁的Id
         CreateGui.getModel().addRoutTran(copy.getId());
      }
      ...
      // 更新是否为路由变迁属性
      copy.isRoutTran = this.isRoutTran;
   ...
   public Transition copy() {
      ...
      copy.setRoutTran(this.isRoutTran);
      if (this.isRoutTran) {// 添加路由变迁的Id
         CreateGui.getModel().addRoutTran(this.getId());
      }
   ...
   public void paintComponent(Graphics g) {
      ...
      if (isRoutTran) {// 若为控制变迁,则设置为蓝色
         Graphics2D g3 = (Graphics2D) g;
         float lineWidth = 4.0f;
         ((Graphics2D) g3).setStroke(new BasicStroke(lineWidth));
         g3.setColor(new Color(33, 192, 165));
         g3.draw(transition);
         g3.setPaint(Pipe.ELEMENT_FILL_COLOUR);
         g3.fill(transition);
      }
...
//            }
   ...
   class ArcAngleCompare implements Comparable {
      ...
      private void calcAngle() {
         ...
         Point2D.Double p1 = new Point2D.Double(positionX + centreOffsetLeft(),
   ...
   public boolean isRoutTran() {
      return isRoutTran;
   }

   public void setRoutTran(boolean isRoutTran) {
      this.isRoutTran = isRoutTran;
   }
