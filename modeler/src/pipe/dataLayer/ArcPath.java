...
public class ArcPath
   ...
   public void createPath() {
      ...
      if (getEndIndex() < 2) {
         midPoint.x = (float) ((((ArcPathPoint) pathPoints.get(0)).getPoint().x
               + ((ArcPathPoint) pathPoints.get(1)).getPoint().x) * 0.4);
         midPoint.y = (float) ((((ArcPathPoint) pathPoints.get(0)).getPoint().y
               + ((ArcPathPoint) pathPoints.get(1)).getPoint().y) * 0.4);
      } else {
         double acc = 0;
         double percent = 0;
   ...
   public double getEndAngle() {
      if (getEndIndex() > 0) {
         if (getArc().getTarget() instanceof Transition) {
            return ((ArcPathPoint) pathPoints.get(getEndIndex()))
                  .getAngle(((ArcPathPoint) (pathPoints.get(getEndIndex()))).getControl2());
         } else {
            return ((ArcPathPoint) pathPoints.get(getEndIndex()))
                  .getAngle(((ArcPathPoint) (pathPoints.get(getEndIndex()))).getControl1());
         }
      }
      return 0;
   }
   ...
   public ArcPathPoint splitSegment(Point2D.Float mouseposition) {
      ...
      ArcPathPoint newpoint = new ArcPathPoint(second.getMidPoint(first), first.getPointType(), this);
