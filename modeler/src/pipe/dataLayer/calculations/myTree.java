...
public class myTree {
   ...
   public void createCoverabilityGraph(RandomAccessFile outputFile,
      ...
      boolean[] enabledTransitions = dataLayer.getTransitionEnabledStatusArray(root.markup);
      ...
      while (!unprocessednodes.isEmpty()) {
         ...
         enabledTransitions = dataLayer.getTransitionEnabledStatusArray(currentNode.markup);
         // For each transition
         for (int i = 0; i < enabledTransitions.length; i++) {
            if (enabledTransitions[i] == true) {
               ...
               currentNode.children[i] = new myNode(newMarkup, currentNode, this, currentNode.depth + 1);
               ...
               if ((!repeatedNode) /* && (!allOmegas) */) {
                  ...
                  if (currentNode.children[i].previousInstance != null) {
                     writeEdge(currentNode.id, currentNode.children[i].previousInstance.id, 0, i, currentNode.markup,
                           outputFile);
                  } else {
      ...
      while (!unprocessednodes.isEmpty()) {
         ...
         for (int i = 0; i < enabledTransitions.length; i++) {
            if (enabledTransitions[i] == true) {
               ...
               //
               // if (currentNode.children[i].previousInstance != null) {
               // writeEdge(currentNode.id, currentNode.children[i].previousInstance.id, 0, i,
               // currentNode.markup, outputFile);
               // } else {
               writeEdge(currentNode.id, currentNode.children[i].id, 0, i, currentNode.markup, outputFile);
               // }
               // } else {
   ...
   private void writeEdge(int from, int to, double rate,
         ...
         throws ImmediateAbortException {
      TransitionRecord newTransition = new TransitionRecord(from, to, rate, transitionNo);
      // System.out.println("writeEdge: from S" + from + " to S" + to + "; t =" +
      // transitionNo); //debug
      try {
