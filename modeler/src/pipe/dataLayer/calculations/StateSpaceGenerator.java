 ...
 *         Stochastic
 *         Petri-Net model. It uses an algorithm developed by Dr. William
 *         Knottenbelt
 *         in his MSc. thesis "Generalised Markovian Analysis of Timed
 *         Transition
 *         Systems" June 1996 Dept. of Computer Science, University of Cape
 *         Town.
 *         The algorithm eliminates vanishing states 'on the fly' and creates
 *         the
...
public class StateSpaceGenerator {
   ...
   public static void generate(DataLayer pnmlData, File reachGraph)
         throws OutOfMemoryError, TimelessTrapException, ImmediateAbortException,
         IOException {
   ...
   public static void generate(DataLayer pnmlData, File reachGraph,
      ...
      if (isTangible(pnmlData, currentMarking)) {
         ...
         while ((!vanishingStates.isEmpty()) && (attempts != MAX_TRIES)) {
            ...
            boolean[] enabledTransitions = pnmlData.getTransitionEnabledStatusArray(v.getState());

            int transition = fireFirstEnabledTransition(pnmlData, enabledTransitions,
                  v, vansuccessor);
            ...
            while (transition != -1) {
               ...
               transition = fireFirstEnabledTransition(pnmlData, enabledTransitions,
                     v, vansuccessor);
            }
         }
      ...
      while (!tangibleStates.isEmpty()) {
         if (progress == UPDATEAFTER) {
            ...
            System.out.print(numtangiblestates + " tangible states generated and " + numtransitionsfired
                  + " transitions fired.\r");
         } else {
            progress++;
         }
   ...
   private static boolean isTangible(DataLayer pnmlData, State marking) {
      ...
      boolean[] enabledTransitions = pnmlData.getTransitionEnabledStatusArray(marking.getState());
   ...
   private static int fire(DataLayer pnmlData, State vs, Stack succ) {
      ...
      boolean[] enabledTransitions = pnmlData.getTransitionEnabledStatusArray(vs.getState());
   ...
   private static double rate(DataLayer pnmlData, State s, State sprime) {
      ...
      boolean[] marking1EnabledTransitions = pnmlData.getTransitionEnabledStatusArray(marking1);
   ...
   private static double prob(DataLayer pnmlData, State s,
      ...
      double candidateTransitionWeighting = pnmlData.getTransitions()[firedTransition].getRate();
      ...
      while (iterator.hasNext()) {
         ...
         if (writeTransitionsNo) {
            ...
            try {
               ...
               // System.out.println("From: " + from.getIDNum() + ":(" + from + ")" + " To: " +
               // current.getTo() + " via " + current.transitionNo+ " Rate: " +
               // current.getRate());
            } catch (IOException e) {
            ...
            try {
               ...
               // System.out.println("From: " + from.getIDNum() + ":(" + from + ")" + " To: " +
               // current.getTo() + " Rate: " + current.getRate());
            } catch (IOException e) {
               System.err.println("IO error when writing transitions to file.");
               throw new ImmediateAbortException();
            }
...
/**
 * @author Nadeem
 *         A simple class used to store arcs between states in the reachability
 *         graph.
