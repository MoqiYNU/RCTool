...
public class CopyPasteManager
   ...
   private void endPaste(GuiView view) {
      ...
      for (int i = 0; i < ptaToPaste.size(); i++) {
         PetriNetObject pnObject = (PetriNetObject) ptaToPaste.get(i).paste(despX, despY, sourceView != view);

         if (pnObject != null) {
            if ((pnObject instanceof Transition) && (sourceView != view)) {
               RateParameter rateParameter = ((Transition) pnObject).getRateParameter();
               if (rateParameter != null) {
                  ((Transition) pnObject).clearRateParameter();
               }
            } else if ((pnObject instanceof Place) && (sourceView != view)) {
               MarkingParameter markingParameter = ((Place) pnObject).getMarkingParameter();
      ...
      for (PetriNetObject pno : ptaToPaste) {
         if ((pno instanceof PlaceTransitionObject)) {
            PlaceTransitionObject pt = ((PlaceTransitionObject) pno).getOriginal().getLastCopy();

            Iterator<Arc> pnoConnectedFromIterator = pt.getConnectFromIterator();
            while (pnoConnectedFromIterator.hasNext()) {
               ...
               Iterator<Arc> pnoConnectedToIterator = pt.getConnectToIterator();
