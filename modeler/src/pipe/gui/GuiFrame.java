 ...
 *         not all example nets were loaded if there was a non .xml file in the
 *         folder
 */
...
public class GuiFrame
   ...
   private FileAction createAction, openAction, closeAction, saveAction,
         ...
         exportTNAction, exportPSAction, importAction, renameAction;
   ...
   public GuiFrame(String title) {
      ...
      // 设置框架的显示图标
      String OsName = System.getProperty("os.name");// 获得操作系统
      // 是Mac就设置dock图标
      if (OsName.contains("Mac")) {
         Image icon_image = new ImageIcon("src/Images/icon.png").getImage();
         // 指定Mac的dock图标(需要导入rt.jar)
      ...
      this.setSize(/* screenSize.width * 80/100 */1311, /* screenSize.height * 80/100 */1016);
      ...
      // statusBar.setFont(new Font("Corbel", Font.PLAIN, 17));
    ...
    *         in URI caused by unescaped blank. The code changes one blank
    *         character
    *         if it exists in the string version of the URL. This way works safely
    *         in
   ...
   private void buildMenus() {
      ...
      // 重新命名当前的Tab
      addMenuItem(fileMenu, renameAction = new FileAction("Rename", "Rename the current tab", ""));
      ...
      try {
         ...
         if (JarUtilities.isJarFile(examplesDirURL)) {
            ...
            ArrayList<JarEntry> nets = JarUtilities.getJarEntries(jarFile, "Example nets");
            ...
            if (nets.length > 0) {
               ...
               JMenu exampleMenu = new JMenu(
                     "Example nets");
               exampleMenu.setIcon(new ImageIcon(
                     Thread.currentThread().getContextClassLoader().getResource(CreateGui.imgPath + "Example.png")));
               int k = 0;
      ...
      addMenuItem(drawMenu, transAction = new TypeAction("Transition", Pipe.IMMTRANS,
...
//            "Add a timed transition", "T", true));
      ...
      addMenuItem(drawMenu, inhibarcAction = new TypeAction("Inhibitor Arc", Pipe.INHIBARC,
            "Add an inhibitor arc", "F", true));
      addMenuItem(drawMenu, annotationAction = new TypeAction("Annotation", Pipe.ANNOTATION,
            "Add an annotation", "N", true));
      drawMenu.addSeparator();
      addMenuItem(drawMenu, tokenAction = new TypeAction("Add token", Pipe.ADDTOKEN, "Add a token", "ADD", true));
      addMenuItem(drawMenu, deleteTokenAction = new TypeAction("Delete token", Pipe.DELTOKEN, "Delete a token",
            "SUBTRACT", true));
      drawMenu.addSeparator();
      addMenuItem(drawMenu, rateAction = new TypeAction("Rate Parameter", Pipe.RATE, "Rate Parameter",
            "R", true));
      addMenuItem(drawMenu, markingAction = new TypeAction("Marking Parameter", Pipe.MARKING, "Marking Parameter",
            "M", true));
      ...
      zoomMenu.setIcon(
            new ImageIcon(Thread.currentThread().getContextClassLoader().getResource(CreateGui.imgPath + "Zoom.png")));
      ...
      addMenuItem(animateMenu, startAction = new AnimateAction("Animation mode", Pipe.START,
            "Toggle Animation Mode", "Ctrl A", true));
      animateMenu.addSeparator();
      addMenuItem(animateMenu, stepbackwardAction = new AnimateAction("Back", Pipe.STEPBACKWARD,
            "Step backward a firing", "4"));
      addMenuItem(animateMenu, stepforwardAction = new AnimateAction("Forward", Pipe.STEPFORWARD,
            "Step forward a firing", "6"));
      addMenuItem(animateMenu, randomAction = new AnimateAction("Random", Pipe.RANDOM,
   ...
   private void buildToolbar() {
      ...
      // 添加重命名
      addButton(toolBar, renameAction);
...
//      addButton(drawingToolBar, timedtransAction);
   ...
   /**
    * @author Ben Kirby
    *         Takes the method of setting up the Zoom menu out of the main
    *         buildMenus method.
   ...
   /**
    * @author Ben Kirby
    *         Just takes the long-winded method of setting up the ComboBox out of
    *         the
...
//      timedtransAction.setEnabled(status);
   ...
   public void saveOperation(boolean forceSaveAs) {
      ...
      if (!forceSaveAs && modelFile != null) { // ordinary save
         /*
          * //Disabled as currently ALWAYS prevents the net from being saved - Nadeem
          * 26/05/2005
   ...
   public void createNewTab(File file, boolean isTN) {
      ...
      if (file == null) {
         ...
         try {
            ...
            if (isTN) {
               ...
               appModel.createFromPNML(// 读取PNML网属性
      ...
      // 添加appTab的弹出菜单
      appTab.addMouseListener(new ShowPopupMenu());

      selectAction.actionPerformed(null);
   }

   // 添加appTab的弹出菜单
   class ShowPopupMenu implements MouseListener {

      @Override
      public void mouseClicked(MouseEvent e) {
         final int index = appTab.getUI().tabForCoordinate(appTab, e.getX(), e.getY());
         if (index != -1) { // Click is on valid tab, not on empty area in tab
            int btn = e.getButton();// 获取鼠标按键
            if (btn == MouseEvent.BUTTON3) { // 右键点击
               final JPopupMenu popupMenu = new JPopupMenu();
               int count = 0;
               // 1.新建Tab
               JMenuItem addTab = new JMenuItem(createAction = new FileAction("New", null, null));
               addTab.setText("New a tab");
               popupMenu.insert(addTab, count++);
               // 2.关闭Tab
               final JMenuItem closeTab = new JMenuItem(closeAction = new FileAction("Close", null, null));
               closeTab.setText("Close the tab");
               popupMenu.insert(closeTab, count++);
               // 3.重命名Tab
               final JMenuItem renameTab = new JMenuItem(renameAction = new FileAction("Rename", null, null));
               renameTab.setText("Rename the tab");
               popupMenu.insert(renameTab, count++);

               final Rectangle tabBounds = appTab.getBoundsAt(index);
               popupMenu.show(appTab, tabBounds.x, tabBounds.y + tabBounds.height);
            }
         }
      }
...
//      timedtransAction.setSelected(mode == Pipe.TIMEDTRANS);
    ...
    *         selected
    *         item is updated to keep track of ZoomActions called from other
    *         sources, a
...
//         }
   ...
   class FileAction extends GuiAction {
      ...
      public void actionPerformed(ActionEvent e) {
         ...
         } else if ((this == renameAction) && (appTab.getTabCount() > 0) && checkForSave()) {
            rename();
   ...
   public void rename() {
      ...
      if (name == null) {// 1.不能为空
         return;
      } else if (name.length() == 0) {// 2.不能为空串
         return;
      } else {// 3.避免重复命名
         if (!RenameUtils.isDuplicatedTabName(name)) {
            int select = CreateGui.getTab().getSelectedIndex();
            if (select == -1) {
               JOptionPane.showMessageDialog(this, "Select a PN first!", "No select", JOptionPane.WARNING_MESSAGE);
            } else {
               CreateGui.getTab().setTitleAt(select, name);
            }
         } else {
            JOptionPane.showMessageDialog(this, "Rename is duplicated!", "Duplicated input",
                  JOptionPane.WARNING_MESSAGE);
         }
      }
   }
