 ...
 *         all class files in the module directory without the need to update
 *         the cfg
 *         files and provide path properties. Modules can now be dropped into
 *         the module
 *         folder and automatically loaded on all subsequent executions of pipe.
 *         Also refactored to reduce number of methods loaded with reflection
 *         into the
 *         Jtree which were subsequently never used. Now only loading the run
 *         method of
...
public class ModuleManager {
    ...
    * 
    *                Matthew - created class filters and now cycle through the
    *                module directory
   ...
   public Vector getModuleClasses(File rootDir) {
      final ExtensionFilter class_files = new ExtensionFilter(Pipe.CLASS_FILE_EXTENSION,
   ...
   private void addClassToTree(Class moduleClass) {
      ...
      if (installedModules.add(moduleClass)) {
         ...
         if (modNode.getChildCount() == 1) {
            Object m = ((DefaultMutableTreeNode) modNode.getFirstChild()).getUserObject();
   ...
   public JTree getModuleTree() {
      ...
      // new DefaultMutableTreeNode("Module Manager");

      // create root children
      load_modules = new DefaultMutableTreeNode("Modules");
      ...
      treeModel = new DefaultTreeModel(load_modules);
      ...
      // 设置模块树中的图标
      IconDefaultTreeCellRenderer renderer = new IconDefaultTreeCellRenderer();
