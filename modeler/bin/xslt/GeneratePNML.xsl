...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
   ...
   <xsl:template match="net/place">
      <xsl:element name="place">
        ...
        <!--  <xsl:call-template name="source"/>
         <xsl:call-template name="sink"/> -->
         <xsl:call-template name="msgPlace"/>
         <xsl:call-template name="resPlace"/>
         <xsl:call-template name="linkPlace"/>
          <xsl:call-template name="dataPlace"/>
  ...
  <!--  <xsl:template name="source">
   ...
   <xsl:template name="sink">
      <xsl:element name = "sink">
         <xsl:value-of select="@sink"/>
      </xsl:element>
   </xsl:template> -->
   
   <xsl:template name="msgPlace">
      <xsl:element name = "msgPlace">
         <xsl:value-of select="@msgPlace"/>
      </xsl:element>
   </xsl:template>  
   
    <xsl:template name="resPlace">
      <xsl:element name = "resPlace">
         <xsl:value-of select="@resPlace"/>
      </xsl:element>
   </xsl:template> 
   
   <xsl:template name="linkPlace">
      <xsl:element name = "linkPlace">
         <xsl:value-of select="@linkPlace"/>
      </xsl:element>
   </xsl:template>  

   <xsl:template name="dataPlace">
      <xsl:element name = "dataPlace">
         <xsl:value-of select="@dataPlace"/>
      </xsl:element>
   </xsl:template>  
   ...
   <xsl:template match="net/labels">
      <xsl:element name="labels">
         ...
         <xsl:element name="finalMarings">
            <xsl:value-of select="@finalMarings"/>
         </xsl:element>
      </xsl:element>
   </xsl:template>
   ...
   <xsl:template match="net/transition">
      <xsl:element name="transition">
         <xsl:call-template name="place-transition"/>
         <xsl:call-template name="routTran"/>
         <xsl:call-template name="orientation"/>
         <xsl:call-template name="myrate"/>
         <xsl:call-template name="timed"/>
         <xsl:call-template name="minDelay"/>
         <xsl:call-template name="maxDelay"/>
   ...
   <xsl:template name="routTran">
      <xsl:element name = "routTran">
         <xsl:value-of select="@routTran"/>
      </xsl:element>
   </xsl:template> 
   
   <xsl:template name="minDelay">
      <xsl:element name = "minDelay">
         <xsl:value-of select="@minDelay"/>
      </xsl:element>
   </xsl:template>  
   
   <xsl:template name="maxDelay">
      <xsl:element name = "maxDelay">
         <xsl:value-of select="@maxDelay"/>
      </xsl:element>
   </xsl:template>   
